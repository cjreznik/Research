...
for m = 2:num_mice
    ...
    min_y0 = 0.5*Volume{m-1}(1);
    max_y0 = 1.5*Volume{m-1}(1);
    ...
    for u = 1:n_pts % do what we did below: solve DE using these parameters and get SSE
        ...
        soln{d-1} = ode23s(@(t,x) ExpDrugModel(t,x,uniform_sobol_scaled(u,1),uniform_sobol_scaled(u,2),uniform_sobol_scaled(u,3), uniform_sobol_scaled(u,4),uniform_sobol_scaled(u,5),uniform_sobol_scaled(u,6)),0:7,[uniform_sobol_scaled(u,7),0,5]);
       ...
       %loop through d times to create d sol vecs
        for d = 2:dosage
          ...
          soln{d} = ode23s(@(t,x) ExpDrugModel(t,x,uniform_sobol_scaled(u,1),uniform_sobol_scaled(u,2),uniform_sobol_scaled(u,3), uniform_sobol_scaled(u,4),uniform_sobol_scaled(u,5),uniform_sobol_scaled(u,6)),t1:t2,[soln{d-1}.y(1,end),soln{d-1}.y(2,end),soln{d-1}.y(3,end)+5]);
...
%%%%%%%%%%%%%%Drug Dosing with best QMC points%%%%%%%%%%%%%%%
   ...
   soln{d-1} = ode23s(@(t,x) ExpDrugModel(t,x,newParam_r_s{m-1}(i),newParam_r_r{m-1}(i),newParam_g{m-1}(i), newParam_lambda_s{m-1}(i),newParam_lambda_r{m-1}(i),newParam_gamma{m-1}(i)),0:7,[y0{m-1}(i),0,5]);
   %fprintf('Ok 1\n'); 
   ...
   %loop through d times to create d sol vecs
        for d = 2:dosage
              soln{d} = ode23s(@(t,x) ExpDrugModel(t,x,newParam_r_s{m-1}(i),newParam_r_r{m-1}(i),newParam_g{m-1}(i), newParam_lambda_s{m-1}(i),newParam_lambda_r{m-1}(i),newParam_gamma{m-1}(i)),t1:t2,[soln{d-1}.y(1,end),soln{d-1}.y(2,end),soln{d-1}.y(3,end)+5]);
        ...
        while tempnewy0 < 0 || tempnewparam_lambda_s < 0 || tempnewparam_g < 0 ||  tempnewparam_gamma < 0  || tempnewparam_lambda_r <0 || tempnewparam_lambda_r > tempnewparam_lambda_s
           ...
           %fprintf('tempnewparam_r_s = %f\n tempnewparamr_r= %f\n tempnewparamr_g = %f\n tempnewparam_lambda_s= %f\n tempnewparam_gamma= %f\n tempnewparam_lambda_r= %f\n tempnewy0= %f\n',tempnewparam_r_s,tempnewparamr_r, tempnewparam_g,tempnewparam_lambda_s, tempnewparam_gamma,tempnewparam_lambda_r, tempnewy0  ) 
            ...
            dosage =  ceil((Days{1,m-1}(end)/7));
             d = 2;
             sol = {};
             sol{d-1} = ode23s(@(t,x) ExpDrugModel(t,x,tempnewparam_r_s,tempnewparamr_r,tempnewparam_g, tempnewparam_lambda_s,tempnewparam_lambda_r,tempnewparam_gamma),0:7,[tempnewy0,0,5]);
             t1 = 7;
             t2 = 14;
            ...
            %loop through d times to create d sol vecs
                for d = 2:dosage
                     ...
                     sol{d} = ode23s(@(t,x) ExpDrugModel(t,x,tempnewparam_r_s,tempnewparamr_r,tempnewparam_g, tempnewparam_lambda_s,tempnewparam_lambda_r,tempnewparam_gamma),t1:t2,[sol{d-1}.y(1,end),sol{d-1}.y(2,end),sol{d-1}.y(3,end)+5]);
...
%for m = 14:15
    ...
    sol{d-1} = ode23s(@(t,x) ExpDrugModel(t,x,newParam_r_s{m-1}(end),newParam_r_r{m-1}(end),newParam_g{m-1}(end), newParam_lambda_s{m-1}(end),newParam_lambda_r{m-1}(end),newParam_gamma{m-1}(end)),0:7,[y0{m-1}(1),0,5]);
    ...
    for d = 2:dosage
     ...
     sol{d} = ode23s(@(t,x) ExpDrugModel(t,x,newParam_r_s{m-1}(end),newParam_r_r{m-1}(end),newParam_g{m-1}(end), newParam_lambda_s{m-1}(end),newParam_lambda_r{m-1}(end),newParam_gamma{m-1}(end)),t1:t2,[soln{d-1}.y(1,end),soln{d-1}.y(2,end),soln{d-1}.y(3,end)+5]);
    ...
    fprintf('For mouse %d, Best_r_s = %f, Best_r_r = %f, Best_g = %f, Best_lambda_s = %f , Best_gamma = %f, Best_lambda_r = %f, Best_SSE = %f, Best_y0 = %f,',m-1, Best_r_s(m-1), Best_r_r(m-1), Best_g(m-1), Best_lambda_s(m-1), Best_gamma(m-1), Best_lambda_r(m-1), Best_SSE(m-1),Best_y0(m-1))
    ...
    save(filename, 'Best_r_s', 'Best_r_r' , 'Best_g',  'Best_lambda_s' , 'Best_gamma' , 'Best_lambda_r','Best_SSE','Best_y0') 
