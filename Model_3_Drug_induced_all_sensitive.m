...
for m = 2:num_mice
    ...
    for u = 1:n_pts % do what we did below: solve DE using these parameters and get SSE
        ...
        soln{d-1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,uniform_sobol_scaled(u,1),uniform_sobol_scaled(u,2),uniform_sobol_scaled(u,3), uniform_sobol_scaled(u,4),uniform_sobol_scaled(u,5),uniform_sobol_scaled(u,6)),0:7,[uniform_sobol_scaled(u,7),0,5]);
...
%%%%%%%%%%%%%%Drug Dosing with best QMC points%%%%%%%%%%%%%%%
   ...
   soln{d-1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,newParam_r_s{m-1}(i),newParam_r_r{m-1}(i),newParam_g{m-1}(i), newParam_lambda_s{m-1}(i),newParam_lambda_r{m-1}(i),newParam_gamma{m-1}(i)),0:7,[y0{m-1}(i),0,5]);
   %fprintf('Ok 1\n'); 
        ...
        while tempnewy0 < 0 || tempnewparam_lambda_s < 0 || tempnewparam_g < 0 ||  tempnewparam_gamma < 0  || tempnewparam_lambda_r <0 || tempnewparam_lambda_r > tempnewparam_lambda_s
           ...
           %fprintf('tempnewparam_r_s = %f\n tempnewparamr_r= %f\n tempnewparamr_g = %f\n tempnewparam_lambda_s= %f\n tempnewparam_gamma= %f\n tempnewparam_lambda_r= %f\n tempnewy0= %f\n',tempnewparam_r_s,tempnewparamr_r, tempnewparam_g,tempnewparam_lambda_s, tempnewparam_gamma,tempnewparam_lambda_r, tempnewy0  ) 
            ...
            dosage =  ceil((Days{1,m-1}(end)/7));
             d = 2;
             sol = {};
             sol{d-1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,tempnewparam_r_s,tempnewparamr_r,tempnewparam_g, tempnewparam_lambda_s,tempnewparam_lambda_r,tempnewparam_gamma),0:7,[tempnewy0,0,5]);
             t1 = 7;
             t2 = 14;
...
%for m = 14:15
    ...
    sol{d-1} = ode23s(@(t,x) ExpDrugModel_Drug_causes_resistance(t,x,newParam_r_s{m-1}(end),newParam_r_r{m-1}(end),newParam_g{m-1}(end), newParam_lambda_s{m-1}(end),newParam_lambda_r{m-1}(end),newParam_gamma{m-1}(end)),0:7,[y0{m-1}(1),0,5]);
